...
const _ = '_' as K, B = 'B' as K, b = 'b' as K,
      G = 'G' as K, g = 'g' as K, W = 'W' as K,
      p = 'p' as K, y = 'y' as K, H = 'H' as K

function f(rows: K[][]): SpriteFrame { return buildFrame(P, rows) }
...
const IDLE_2: K[][] = IDLE_1.map((row, i) => {
  if (i === 16) return [_,_,_,_,_,_,_,_,B,B,B,B,B,B,B,B,B,B,B,B,B,B,_,b,b,b,_,_,_,_]
  if (i === 17) return [_,_,_,_,_,_,_,_,B,B,B,B,B,B,B,B,B,B,B,B,B,B,_,b,_,b,_,_,_,_]
  if (i === 18) return [_,_,_,_,_,_,_,_,B,B,B,B,B,B,B,B,B,B,B,B,B,B,_,b,b,b,_,_,_,_]
  return row
})
...
const WALK_R2: K[][] = WALK_R1.map((row, i) => {
  if (i === 21) return [_,_,_,_,_,_,_,_,B,B,b,_,_,_,_,_,_,b,B,B,_,_,_,_,_,_,_,_,_,_]
  if (i === 22) return [_,_,_,_,_,_,_,_,B,b,_,_,_,_,_,_,_,_,b,B,_,_,_,_,_,_,_,_,_,_]
  if (i === 23) return [_,_,_,_,_,_,_,_,b,b,_,_,_,_,_,_,_,_,b,b,_,_,_,_,_,_,_,_,_,_]
  return row
})
...
const HEART_FRAME: K[][] = Array.from({ length: 30 }, (_el, i): K[] => {
  ...
  if (i === 8)  { row[11]=H; row[12]=H; row[15]=H; row[16]=H; row[17]=H; row[18]=H }
  if (i === 9)  { for (let c=10; c<=19; c++) row[c]=H }
  if (i === 10) { for (let c=10; c<=19; c++) row[c]=H }
  if (i === 11) { for (let c=11; c<=18; c++) row[c]=H }
  if (i === 12) { for (let c=12; c<=17; c++) row[c]=H }
  if (i === 13) { for (let c=13; c<=16; c++) row[c]=H }
  if (i === 14) { row[14]=H; row[15]=H }
  return row
})
