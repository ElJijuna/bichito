import type { PetDefinition, SpriteFrame, PixelRow } from '../types.js'
...
const WALK_R1: K[][] = IDLE_1.map((row, i) => {
  if (i === 21) return [_,_,_,_,_,_,_,_,_,b,B,B,_,_,_,_,_,_,B,B,_,_,_,_,_,_,_,_,_,_]
  if (i === 22) return [_,_,_,_,_,_,_,_,_,b,W,W,_,_,_,_,_,_,W,W,_,_,_,_,_,_,_,_,_,_]
  if (i === 23) return [_,_,_,_,_,_,_,_,_,b,w,w,_,_,_,_,_,_,w,w,_,_,_,_,_,_,_,_,_,_]
  return row
})

const WALK_R2: K[][] = IDLE_1.map((row, i) => {
  if (i === 21) return [_,_,_,_,_,_,_,_,_,_,B,B,_,_,_,_,_,_,B,B,b,_,_,_,_,_,_,_,_,_]
  if (i === 22) return [_,_,_,_,_,_,_,_,_,_,W,W,_,_,_,_,_,_,W,W,b,_,_,_,_,_,_,_,_,_]
  if (i === 23) return [_,_,_,_,_,_,_,_,_,_,w,w,_,_,_,_,_,_,w,w,b,_,_,_,_,_,_,_,_,_]
  return row
})
